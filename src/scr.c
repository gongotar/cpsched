...
/*
+++++++++++++++++++++++++++++++
start changed by gholami
functions to communicate with scheduler
+++++++++++++++++++++++++++++++
*/

/* +++++++++++++++++++++++++++++
 * this part goes to the config file
 * +++++++++++++++++++++++++++++
 */
 
/* default duration between two checkpoint requests in milliseconds*/
const char* DEFAULT_NATURAL_INTERVAL = "1000";
/* scheduler server host address*/
const char* SCHEDULER = "130.73.144.90";
/* scheduler server host port*/
const int PORT = 3030;
...
/* saved connection to scheduler*/
int scheduler_conn;
/* id of this job*/
char* jobid;
/* received description from scheduler*/
scr_reddesc* received_desc;
...
int scheduler_server_connect() {
	...
	server = gethostbyname(SCHEDULER);
...
/* sends the given data to the given server connection*/
int send_data(int conn, char *data, int len, char* result) {
	/* convert data to proper format*/
	for (int i = 0; i < len; i++) {
		data[i] = ((int)data[i]) & 0xFF;
	}
	
	int n = write(conn,data,len);
	if (n < 0) {
...
/* extracts the message and returns the correct description sent by the scheduler*/
scr_reddesc* extract_desc_from_message(char* msg, scr_reddesc* descs) {
	return NULL;
}

/* receives instruction of permitted checkpointing level of a specific 
checkpoint from the scheduler.*/
scr_reddesc* get_desc_from_scheduler(int ckpt_id, scr_reddesc* descs) {
	char* ans = malloc(64*sizeof(char));
	
	if(scr_my_rank_world == 0) {
		/* gather all needed infos by the scheduler for decision making*/
		char* nodename = scr_my_hostname;
		int mpi_rank = scr_my_rank_world;
		
		/* pack all gathered infos into a message*/
		char message[256];
		sprintf(message, "cpr#%s#%s#%d", jobid, nodename, mpi_rank);
		
		/* send infos to scheduler*/
		int res = send_data(scheduler_conn, message, strlen(message), ans);
		if(res < 0) {
			error("ERROR sending checkpoint request or receiving decision from scheduler");
		}
		
	}
	
	/* rank 0 broadcasts the decision */
  //MPI_Bcast(ans, 64, MPI_CHAR, 0, scr_comm_world);
	
	/* extract the correct descriptio from scheduler answer*/
	scr_reddesc* desc = extract_desc_from_message(ans, descs);
	
  return desc;
}

/* initializes the scheduler server to know about the new job*/
int initialize_scheduler() {
	
	int ret = -1;
	/* root mpi gathers all needed infos by the scheduler to be sent*/
	if(scr_my_rank_world == 0) {
		char* cp_size = getenv("SCR_CHECKPOINT_SIZE");
		char* nat_intv = getenv("SCR_NATURAL_INTERVAL");

		if(cp_size == NULL) {
			scr_abort(-1, "Checkpoint size is not set; set SCR_CHECKPOINT_SIZE @ %s:%d",
				__FILE__, __LINE__
			);
		}
		if(nat_intv == NULL) {
			strcpy(nat_intv, DEFAULT_NATURAL_INTERVAL);
		}
		
		jobid = getenv("SCR_JOB_ID");
		char* job_stepid = getenv("SLURM_STEPID");
		char* jobnodes_no = getenv("SLURM_JOB_NUM_NODES");
		char* jobnodes = getenv("SLURM_JOB_NODELIST");
		int mpi_size = scr_ranks_world;
		
		/* pack all gathered infos into a single message*/
		char message[512];
    sprintf(message, "init#%s#%s#%s#%s#%d#%s#%s", jobid, job_stepid, jobnodes_no, jobnodes, mpi_size, cp_size, nat_intv);
		
		/* connect to the scheduler server*/
		scheduler_conn = scheduler_server_connect();
		if(scheduler_conn > 0) {
			/* send infos to scheduler*/
			char* ans = malloc(64*sizeof(char));
			int res = send_data(scheduler_conn, message, strlen(message), ans);
			if(res > 0 && strcmp(ans, "done") == 0) {
				ret = 1;
			}
		}
	}

	/* rank 0 broadcasts the initialization result */
  MPI_Bcast(&ret, 1, MPI_INT, 0, scr_comm_world);
	
	return ret;
}

/* finalizes the scheduler so that it knows about the job termination/completion*/
int finalize_scheduler() {
...
static scr_reddesc* scr_get_reddesc(const scr_dataset* dataset, int ndescs, scr_reddesc* descs)
{
  ...
  if (is_ckpt) {
    ...
    if (scr_dataset_get_ckpt(dataset, &ckpt_id) == SCR_SUCCESS) {
      ...
      +++++++++++++++++++++++++++++++
      start changed by gholami
      get description from scheduler
      +++++++++++++++++++++++++++++++
      */
      ...
      // d = get_desc_from_scheduler(ckpt_id, descs);
...
static int scr_start_output(const char* name, int flags)
{
  ...
  /* get the redundancy descriptor for this dataset */
	...
	scr_reddesc* reddesc;
	if(received_desc == NULL) {
		reddesc = scr_get_reddesc(dataset, scr_nreddescs, scr_reddescs);
	}
	else {	/* if using scheduler then use the results from scheduler*/
		*reddesc = *received_desc;
		received_desc = NULL;
	}
...
int SCR_Init()
{
  ...
  scr_initialized = 1;
	...
	*/
	...
	int res = initialize_scheduler();
	if(res < 0) {
		scr_abort(-1, "Failed to initialize the scheduler @ %s:%d",
          __FILE__, __LINE__
        );
	}
  ...
  if (scr_my_rank_world == 0) {
        ...
        scr_checkpoint_overhead <= 0)
    {
      *flag = 1;
		...
		received_desc = get_desc_from_scheduler(scr_checkpoint_id + 1, scr_reddescs);
		if(received_desc == NULL) {
			*flag = 0;
		}
		else {
